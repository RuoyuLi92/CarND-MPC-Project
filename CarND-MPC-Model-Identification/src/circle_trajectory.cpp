...
int main() {
  ...
  MPC mpc;
  // Prediction horizon 
  ofstream xout("output/x_vals.txt");
  ofstream yout("output/y_vals.txt");
  ofstream psiout("output/psi_vals.txt");
  ofstream vout("output/v_vals.txt");
  xout<<"x_vals:"<<endl;
  yout<<"y_vals:"<<endl;
  psiout<<"psi_vals:"<<endl;
  vout<<"v_vals:"<<endl;
  xout.close();
  yout.close();
  psiout.close();
  ...
  h.onMessage([&mpc](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
    ...
    std::cout << "Recieved data from simulator " << sdata << std::endl;
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          double psi = j[1]["psi"];
          double v = j[1]["speed"];
          ofstream xout("output/x_vals.txt", ios::app);
          ofstream yout("output/y_vals.txt", ios::app);
          ofstream psiout("output/psi_vals.txt", ios::app);
          ofstream vout("output/v_vals.txt", ios::app);
          xout<< px << " ";
          yout<< py << " ";
          psiout<< psi << " ";
          vout<< v << " ";
