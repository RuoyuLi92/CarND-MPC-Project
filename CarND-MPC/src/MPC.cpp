...
double throttle_start = delta_start + N - 1;
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
     ...
     */
	 ...
	 // minimize the value gap bewteen the sequential actuations
	 for ( int i = 1; i < N-1; ++i) {
		 fg[0] += 5000*CppAD::pow(vars[delta_start + i] - vars[delta_start + i -1], 2);
		 fg[0] += CppAD::pow(vars[throttle_start + i] - vars[throttle_start + i -1], 2);
	 }
	 
	 // define the constraints x_t+1 = x_t + vx * dt ==> fg[t] = x[t+1] - (x[t] + vx * dt) == 0 
	 ...
	 for ( int i = 1; i < N; ++i) {
		 ...
		 AD<double> throttle0 = vars[throttle_start + i -1];
		 ...
		 fg[1 + epsi_start + i] = epsi1 - (psides0 - psi0 + v0 / Lf * delta0 * dt);
	 }		 
...
std::vector<double> MPC::Solve(const VectorXd &state, const VectorXd &coeffs) {
  ...
  for (int i = throttle_start; i < throttle_start + N-1; ++i) {
   ...
   */
  std::cout<<"before vars_out"<<std::endl;
  std::vector<double> vars_out(2*N);
  
  for (int i = x_start + 1; i < x_start + N; ++i) {
	  vars_out[i-1] = solution.x[i];
  }
  for (int i = y_start + 1; i < y_start + N; ++i) {
	  vars_out[i-2] = solution.x[i];
  }
  vars_out[2*N-2] = solution.x[delta_start];
  vars_out[2*N-1] = solution.x[throttle_start];
  std::cout<<"after vars_Out"<<std::endl;
  std::cout<<vars_out[2*N-2]<<std::endl;
  std::cout<<vars_out[2*N-1]<<std::endl;
  return vars_out;
}
